...
{
	private static final String DATABASE_NAME = MakanApaConfig.DATABASE_NAME;
	private static final String DATABASE_PATH = "/data/data/" + MakanApaApplication.getContext().getPackageName() + "/databases/";
	private static final int DATABASE_VERSION = MakanApaConfig.DATABASE_VERSION;
	...
	{
		super(MakanApaApplication.getContext(), DATABASE_PATH + DATABASE_NAME, null, DATABASE_VERSION);
	...
	private boolean copyPrepopulatedDatabase()
	{
		...
		try
		{
			...
			// create streams
			InputStream inputStream = MakanApaApplication.getContext().getAssets().open(DATABASE_NAME);
	...
	{
		SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(MakanApaApplication.getContext());
